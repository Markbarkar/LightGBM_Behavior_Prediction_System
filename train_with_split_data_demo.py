...
"""
基于LightGBM算法的用户行为预测模型 - 使用分割数据集训练（演示版）

该脚本使用已经分割好的训练集、测试集和验证集的采样数据训练LightGBM模型，
并绘制训练过程中的模型性能变化图表。
"""
...
def parse_args():
    ...
    parser.add_argument('--num_boost_round', type=int, default=100, help='最大迭代次数')
    parser.add_argument('--early_stopping_rounds', type=int, default=20, help='早停轮数')
    parser.add_argument('--sample_size', type=int, default=100000, help='每个数据集的采样大小')
...
def train_lightgbm_model(X_train, y_train, X_val, y_val, feature_names, categorical_features=None, params=None):
    ...
    model = lgb.train(
        ...
        valid_names=['train', 'valid'],
        callbacks=[
            early_stopping(stopping_rounds=params.get('early_stopping_rounds', 20)),
...
def main():
    ...
    # 加载数据（使用采样以加快处理速度）
    train_df = load_data(args.train_data, args.sample_size)
    val_df = load_data(args.val_data, args.sample_size)
    test_df = load_data(args.test_data, args.sample_size)
    ...
    # 保存模型
    model_path = os.path.join(args.model_dir, 'lightgbm_user_behavior_model_demo.txt')
